...
public class Chys_html {
    ...
    public ResponseEntity<?> update(@PathVariable("id") int id, @RequestBody dtohys_html dtohtml){
        ...
        //if(shys_html .existsByPorcentualE(dtohtml.getPorcentualE()) && shys_html .getByPorcentualE(dtohtml.getPorcentualE()).get().getId() != id)
        //    return new ResponseEntity(new Mensaje("Esa experienci ya existe"), HttpStatus.BAD_REQUEST);
        //No puede estar vacio
        //if(StringUtils.isBlank(dtohtml.getPorcentualE())) //para hacerlo int
          //  return new ResponseEntity(new Mensaje("El nombre es obligatorio"), HttpStatus.BAD_REQUEST);  //para hacerlo int
